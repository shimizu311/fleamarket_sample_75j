...
RSpec.describe Item, type: :model do
  describe '#update' do
    ...
    it "配送料の負担が選択されてなければ更新できないこと" do
      item = @item
    ...
    it "発送元の地域が選択されてなければ更新できないこと" do
      item = @item
    ...
    it "発送までの日数が選択されてなければ更新できないこと" do
      item = @item
    ...
    it "seller_idがnullの場合は更新できないこと" do
      item = @item
    ...
    it "商品コメントが1000文字であれば更新できること" do
      item = @item
      item = build(:item, text: "a" * 1000)
      expect(item[:text])
    end

    it "商品コメントが1001文字であれば更新できないこと" do
      item = @item
      item = build(:item, text: "a" * 1001)
      expect(item.errors[:text])
    end

    it "商品価格が数値以外の場合は更新できないこと" do
      item = @item
    ...
    it "商品価格が9999999円の場合は更新できること" do
      item = @item
    ...
    it "商品価格が10000000円の場合は更新できないこと" do
      item = @item
